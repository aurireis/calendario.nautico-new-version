...
function googleCalendarUrl(ev){
  const start = parseCalendarDate(ev.s);
  if(!start) return '';

  let end = parseCalendarDate(ev.e || ev.s) || start;
  if(end < start) end = start;
  end = new Date(end.getTime() + 86400000); // Google usa a data final exclusiva em eventos de dia inteiro

  const cat = CATS[ev.c] || CATS.outros;
  const details = [
    `Data: ${fmtWhen(ev)}`,
    `Categoria: ${cat.label}`,
    ev.tbd ? 'Data sujeita a confirmação.' : '',
    'Calendário de Eventos Náuticos 2026 — Secretaria Especial do Mar, Salvador.'
  ].filter(Boolean).join('\n');
  const params = new URLSearchParams({
    action: 'TEMPLATE',
    text: String(ev.t || 'Evento náutico'),
...
function renderAll(){
  ...
  MESES.forEach((mesName, mi)=>{
    ...
    items.forEach(ev=>{
      ...
      const calendarUrl = googleCalendarUrl(ev);
      html += `<div class="event ${isToday?'today-marker':''} ${isPast?'is-past':''}" style="--dot-color:${cat.color}">
        <div class="event-grid">
          <div class="ev-date">${ev.tbd ? '<span class="tbd">'+esc(ev.d)+'</span>' : esc(ev.d)}</div>
          <div class="ev-body">
            <div class="ev-title">${esc(ev.t)}</div>
            <div class="ev-meta">
              <span class="tag" style="--tag-color:${cat.color}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${cat.icon}</svg>${esc(cat.label)}</span>
              <span class="status ${st}">${statusLabel(st)}</span>
            </div>
            ${!isPast ? `<div class="event-actions">
              <button class="notify-btn" data-title="${esc(ev.t)}" data-when="${esc(fmtWhen(ev))}" data-date="${esc(ev.s || '')}">
              ...
              ${calendarUrl ? `<a class="calendar-btn" href="${esc(calendarUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Adicionar ${esc(ev.t)} ao Google Agenda">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M16 3v4M8 3v4M3 10h18"/><path d="M12 13v5M9.5 15.5h5"/></svg>
...
function updateContactField(){
  if (contactLabel) contactLabel.textContent = 'Seu e-mail';
  if (contactInput) {
    contactInput.type = 'email';
    contactInput.placeholder = 'voce@email.com';
  }
  if (contactHint) contactHint.textContent = 'Você receberá um lembrete por e-mail um dia antes do evento.';
  if (submitBtn) {
...
document.getElementById('notifyForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    showToast('Prontinho!', 'Você vai receber um lembrete um dia antes do evento.');
    closeModal();
    form.reset();
  } catch (err) {
    console.error(err);
    showToast('Ops!', 'Não deu pra confirmar agora. Tenta de novo em instantes.');
  } finally {
...
/* ---------------- search / toggle / clock / top ---------------- */
const searchInput = document.getElementById('searchInput');
if (searchInput) searchInput.addEventListener('input', e=>{ searchTerm = e.target.value.trim().toLowerCase(); renderAll(); });

const onlyUpcomingCheck = document.getElementById('onlyUpcoming');
if (onlyUpcomingCheck) onlyUpcomingCheck.addEventListener('change', e=>{ onlyUpcoming = e.target.checked; renderAll(); });
