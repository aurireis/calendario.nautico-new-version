...
- **XSS corrigido**: dados digitados por visitantes (nome, contato, título do
  evento no formulário público) agora são escapados antes de aparecer no
  dashboard — antes, um visitante malicioso podia injetar código que rodaria
...
- **Validação de entrada**: e-mail e telefone são validados por formato antes
  ...
  máximo.
- **Rate limiting**: no máximo 5 pedidos de alerta por hora para o mesmo
  contato e 20 por hora por IP — impede que alguém use sua conta Gmail/Twilio
  como disparador de spam em massa. O login do dashboard também é limitado
  (8 tentativas de senha por 15 min por IP), contra força bruta.
...
- **Cabeçalhos HTTP de segurança** (`netlify.toml`): `X-Frame-Options`,
  ...
  `noindex` + `no-store` específicos pro dashboard.
